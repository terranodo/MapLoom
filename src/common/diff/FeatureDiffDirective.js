(function () {
  ...
  module.directive('loomFeatureDiff', [
    ...
    'dialogService',
    'geogigService',
    function ($rootScope, $translate, featureDiffService, conflictService, configService, historyService, notificationService, featureBlameService, mapService, dialogService, geogigService) {
      return {
        restrict: 'C',
        ...
        link: function (scope, element) {
          ...
          var undo = function () {
            ...
            options.commitMessage = $translate.instant('reverted_changes_to_feature', { feature: featureDiffService.feature.id }) + ' ' + $translate.instant('applied_via_maploom');
              ...
              transaction.command('revertfeature', options).then(function () {
                transaction.finalize().then(function () {
                  ...
                  if (goog.isObject(endTransactionFailure) && goog.isDefAndNotNull(endTransactionFailure.conflicts)) {
                    handleConflicts(repoId, endTransactionFailure, transaction, dialogService, conflictService, $translate, $translate.instant('transaction'), $translate.instant('repository'), scope, $translate.instant('transaction'));
                  } else {
                ...
                if (goog.isObject(mergeFailure) && goog.isDefAndNotNull(mergeFailure.conflicts)) {
                  handleConflicts(repoId, mergeFailure, transaction, dialogService, conflictService, $translate, branch, $translate.instant('fixed_feature'), scope, $translate.instant('fixed_feature'));
                } else {
