      ...
      return {
        ...
        link: function (scope, element) {
            ...
            if (goog.isDefAndNotNull(files)) {
              ...
              for (var i = 0; i < files.length; i++) {
                ...
                if (goog.isDefAndNotNull(file)) {
                  fileUpload.uploadFileToUrl(file, configService.configuration.fileserviceUploadUrl, configService.csrfToken).then(onSuccess, onReject);
                }
              }
            }
          };
          ...
          scope.save = function () {
            ...
            if (numErrors > 0) {
              dialogService.warn($translate.instant('save_attributes'), $translate.instant('invalid_fields', { value: numErrors }), [$translate.instant('btn_ok')], false);
              return;
              ...
              dialogService.open($translate.instant('location_lon_lat'), $translate.instant('latlon_confirm', { value: shortXYValue(scope.coordinates.coords) }), [
                ...
                case 0: {
                    scope.isSaving = true;
                    featureManagerService.endAttributeEditing(true, scope.inserting, scope.properties, scope.coordinates.coords).then(function (resolve) {
                      reset();
                      $('#attribute-edit-dialog').modal('toggle');
                      scope.isSaving = false;
                    }, function (reject) {
                      scope.isSaving = false;
                      var message = scope.inserting ? 'unable_to_save_feature' : 'unable_to_save_attributes';
                      dialogService.error($translate.instant('error'), $translate.instant(message, { 'value': reject }), [$translate.instant('btn_ok')], false);
                    });
                  }
                }
              });
              return;
            } else if (goog.isDefAndNotNull(scope.coordinates) && scope.coordinates.changed && scope.coordDisplay.value === coordinateDisplays.MGRS && xyToMGRSFormat(scope.coordinates.coords4326) !== scope.coordinates.originalText) {
              dialogService.open($translate.instant('location_lon_lat'), $translate.instant('latlon_confirm', { value: shortXYValue(scope.coordinates.coords) }), [
                $translate.instant('yes_btn'),
                $translate.instant('no_btn')
              ], false).then(function (button) {
            ...
            scope.isSaving = true;
            featureManagerService.endAttributeEditing(true, scope.inserting, scope.properties, goog.isDefAndNotNull(scope.coordinates) ? scope.coordinates.coords : scope.coordinates).then(function (resolve) {
              reset();
              $('#attribute-edit-dialog').modal('toggle');
              scope.isSaving = false;
            }, function (reject) {
