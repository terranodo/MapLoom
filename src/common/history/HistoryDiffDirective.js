(function () {
  ...
  module.directive('loomHistoryDiff', [
    ...
    'dialogService',
    '$window',
    function ($rootScope, historyService, $translate, geogigService, diffService, pulldownService, dialogService, $window) {
      return {
                ...
                diffService.performDiff(historyService.repoId, diffOptions).then(function (response) {
                  if (goog.isDefAndNotNull(response.Feature)) {
                    if (goog.isDefAndNotNull(response.nextPage) && response.nextPage == 'true') {
                      dialogService.warn($translate.instant('warning'), $translate.instant('too_many_changes'), [$translate.instant('btn_ok')]);
                      scope.isLoading = false;
                    } else {
                    ...
                    dialogService.open($translate.instant('history'), $translate.instant('no_changes_in_time_range'), [$translate.instant('btn_ok')]);
                    scope.isLoading = false;
                  }
                }, function (reject) {
                  console.log('Failed to get diff: ', reject);
                  dialogService.error($translate.instant('error'), $translate.instant('diff_unknown_error'), [$translate.instant('btn_ok')]);
                  scope.isLoading = false;
                });
              } else {
                dialogService.open($translate.instant('history'), $translate.instant('no_changes_in_time_range'), [$translate.instant('btn_ok')]);
