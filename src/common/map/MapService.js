(function () {
  ...
  function createGeoJSONLayerFromCoordinatesWithProjection(coordinates, projection) {
    ...
    return new ol.layer.Vector({ source: new ol.source.GeoJSON({ object: geojsonObject }) });
  }
  module.provider('mapService', function () {
    this.$get = [
      ...
      '$rootScope',
      '$q',
      function ($translate, serverService, geogigService, $http, pulldownService, $cookieStore, $cookies, configService, dialogService, tableViewService, $rootScope, $q) {
        service_ = this;
        httpService_ = $http;
    ...
    this.zoomToExtentForProjection = function (extent, projection) {
      var transform = ol.proj.getTransformFromProjections(projection, this.map.getView().getProjection());
    ...
    this.zoomToLayerExtent = function (layer) {
      ...
      var shrinkExtent = function (extent, shrink) {
        ...
        if (!goog.isDefAndNotNull(extent) && goog.isDefAndNotNull(metadata) && goog.isDefAndNotNull(metadata.bbox.crs)) {
        ...
        var transform = ol.proj.getTransformFromProjections(ol.proj.get(metadata.bbox.crs), service_.map.getView().getProjection());
        ...
        if (goog.isDefAndNotNull(projcode)) {
          ...
          if (!goog.isDefAndNotNull(prj)) {
            ...
            try {
              ...
              console.log('====[ NOTE: proj file is downloaded and added to proj4 but we need to either do syncronous call or make sure ' + 'it is resolved before we continue on or return a promis and make sure all invokaction of addLayer user ' + 'the promise properly ', projcode);
            }
          }
        }
      }
      ...
      if (!goog.isDefAndNotNull(fullConfig)) {
        ...
        if (fullConfig.type && fullConfig.type == 'mapproxy_tms') {
          ...
          layer = new ol.layer.Tile({
            ...
            visible: true,
            source: new ol.source.XYZ({ url: fullConfig.detail_url + '/map/wmts/' + layername + '/default_grid/{z}/{x}/{y}.png' })
          });
        } else if (server.ptype === 'gxp_osmsource') {
          var osmLocal = {
              attributions: [
                new ol.Attribution({ html: settings.OsmLocalAttribution }),
          ...
          dialogService_.error(translate_.instant('add_layers'), translate_.instant('layer_type_not_supported', { type: 'gxp_googlesource' }));
          ...
          dialogService_.error(translate_.instant('add_layers'), translate_.instant('layer_type_not_supported', { type: 'gxp_olsource' }));
        }
      }
      return layer;
    };
    ...
    this.save = function (copy) {
      ...
      goog.array.forEach(service_.getLayers(true, true), function (layer, key, obj) {
        ...
        if (!goog.isDefAndNotNull(config)) {
          console.log('Not saving layer: ', layer.get('metadata').name, 'because the layer does not have a configuration object.');
          return false;
        }
      ...
      httpService_({
        ...
        if (status == 403 || status == 401) {
          ...
          dialogService_.error(translate_.instant('save_failed'), translate_.instant('map_save_failed', { value: status }));
        }
      });
    };
      ...
      if (goog.isDefAndNotNull(service_.configuration) && goog.isDefAndNotNull(service_.configuration.sources) && goog.isDefAndNotNull(service_.configuration.map) && goog.isDefAndNotNull(service_.configuration.map.layers)) {
            ...
            if (serverInfo.lazy === true) {
              for (var layerIndex = 0; layerIndex < service_.configuration.map.layers.length; layerIndex++) {
                ...
                if (mapLayer.source === key.toString() && goog.isDefAndNotNull(mapLayer.name)) {
                  console.log('====[ Note: Server is marked as lazy, but a map layer depends on the server.  ' + 'Will ignore lazy flag.', serverInfo, mapLayer);
        ...
        goog.array.forEach(orderedUnique, function (serverInfo, serverIndex, obj) {
          if (goog.isDefAndNotNull(serverInfo)) {
            serverService_.addServer(serverInfo, true).then(function (serverNew) {
              orderedUniqueLength--;
