(function () {
  ...
  module.directive('loomMerge', [
    ...
    'featureDiffService',
    'configService',
    function ($translate, geogigService, dialogService, notificationService, historyService, conflictService, mapService, featureDiffService, configService) {
      return {
                    ...
                    transaction.command('commit', commitOptions).then(function (commitResponse) {
                      transaction.finalize().then(function () {
                        ...
                        if (goog.isObject(endTransactionFailure) && goog.isDefAndNotNull(endTransactionFailure.conflicts)) {
                          handleConflicts(endTransactionFailure, transaction, dialogService, conflictService, $translate, $translate.instant('transaction'), $translate.instant('repository'), scope, $translate.instant('transaction'));
                        } else {
