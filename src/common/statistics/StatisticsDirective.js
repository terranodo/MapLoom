(function () {
  ...
  module.directive('loomStatisticsView', function () {
    return {
      restrict: 'C',
  ...
  module.directive('loomStatisticsChart', function () {
    return {
      restrict: 'EA',
      link: function (scope, element, attrs) {
        scope.$watch('data', function (newVals, oldVals) {
          if (goog.isDefAndNotNull(newVals) && goog.isDefAndNotNull(newVals)) {
            ...
            if (goog.isDefAndNotNull(scope.data) || goog.isDefAndNotNull(scope.data.statistics) && goog.isDefAndNotNull(scope.data.statistics.uniqueValues)) {
        ...
        scope.renderHistogram = function () {
          var data = scope.data;
          if (!data || !(goog.isDefAndNotNull(data.statistics) && goog.isDefAndNotNull(data.statistics.uniqueValues))) {
            return;
          }
          scope.clearData();
          ...
          var yScale = d3.scale.linear().domain([
              scope.uniqueValueMin,
              scope.uniqueValueMax
            ]).range([
              chartHeight,
              scope.margin.bottom
            ]);
          var xScale = d3.scale.ordinal().domain(d3.keys(data.statistics.uniqueValues)).rangeBands([
              scope.margin.left + 10,
              chartWidth
            ], 0.25, 0.25);
          var sliceHoverClass = 'path-red-fill';
          var xAxisLabelCollision = goog.isDefAndNotNull(scope.maxLabelLength) && 7 * 0.4 * scope.maxLabelLength + 20 > xScale.rangeBand();
          ...
          var svg = d3.select(element[0]).append('div').attr('id', 'loom-statistics-histogram').attr('width', scope.divWidth).append('svg:svg').attr('width', scope.divWidth);
          svg.attr('height', scope.divHeight);
          var yAxis = d3.svg.axis().scale(yScale).orient('left');
          svg.append('g').attr('class', 'y axis').attr('transform', 'translate(' + scope.margin.left + ',0)').call(yAxis).selectAll('text').classed('no-select', true);
          svg.selectAll('g').filter(function (d) {
            return d;
          }).classed('minor', true);
          svg.selectAll('rect').data(histogram).enter().append('rect').attr('id', function (d, index) {
            return 'histogram-chart-slice-path-' + index;
          }).classed('black-stroke', true).attr('width', xScale.rangeBand()).attr('y', function (d) {
            ...
            return chartHeight - yScale(d.value) + 1;
          });
          var xAxis = d3.svg.axis().scale(xScale).orient('bottom').tickPadding(10).tickFormat(function (d) {
          ...
          svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + (chartHeight + scope.margin.bottom) + ')').call(xAxis).selectAll('text').classed('no-select x-axis-label', true);
          if (xAxisLabelCollision) {
            svg.selectAll('.x-axis-label').attr('transform', 'rotate(90)').style('text-anchor', 'start').attr('y', 0).attr('x', 9).attr('dy', '.35em');
          }
        ...
        scope.renderPieChart = function () {
          ...
          var svg = d3.select(element[0]).append('div').attr('id', 'loom-statistics-pie').attr('width', scope.divWidth).append('svg:svg').attr('width', chartWidth).attr('height', chartHeight).attr('id', 'pie-chart').data([uniqueValues]).append('svg:g').attr('transform', 'translate(' + chartWidth / 2 + ',' + chartHeight / 2 + ')').style('margin-left', d3.select('.statistics-legend').node().offsetWidth);
          var pie = d3.layout.pie().value(function (d) {
              return d.value;
            });
          var arc = d3.svg.arc().outerRadius(r);
          var arcs = svg.selectAll('g.slice').data(pie).enter().append('g').attr('class', 'arc');
          arcs.append('path').attr('fill', function (d, i) {
            return color(i);
          }).classed('pie-chart-slice black-stroke', true).attr('id', function (d, index) {
        ...
        scope.renderLegend = function (data) {
          ...
          var color = scope.color();
          var legendDiv = d3.select(element[0]).append('div').attr('class', 'statistics-legend');
          var legend = legendDiv.append('table').attr('class', 'statistics-legend-table');
          var tr = legend.append('tbody').selectAll('tr').data(data).enter().append('tr').attr('id', function (d, index) {
          ...
          }).classed('statistics-legend-text no-select', true).attr('data-toggle', 'tooltip').attr('data-placement', 'right').attr('title', function (d) {
