(function () {
  ...
  module.provider('synchronizationService', function () {
    this.$get = [
      ...
      'conflictService',
      'configService',
      function ($timeout, $rootScope, $q, $translate, dialogService, historyService, geogigService, conflictService, configService) {
    ...
    this.addLink = function (link) {
      for (var index = 0; index < synchronizationLinks_.length; index++) {
        if (synchronizationLinks_[index].equals(link) || link.name === synchronizationLinks_[index].name) {
          dialogService_.open(translate_.instant('add_sync'), translate_.instant('link_already_exists'), [translate_.instant('btn_ok')], false);
          return;
        }
      }
      link.id = nextLinkId_;
      ...
      geogigService_.beginTransaction(link.getRepo().id).then(function (transaction) {
        ...
        transaction.command('pull', pullOptions).then(function (pullResult) {
          ...
          if (goog.isObject(pullFailed) && goog.isDefAndNotNull(pullFailed.conflicts)) {
            ...
            handleConflicts(pullFailed, transaction, link.getRepo().id, translate_.instant('local'), link.getRemote().name, branch);
            result.reject(false);
          } else {
            if (!errorMessageOn) {
              if (link.continuous) {
            ...
            if (showMessage) {
              errorMessageOn = true;
              dialogService_.error(translate_.instant('error'), translate_.instant(message), [translate_.instant('btn_ok')], false).then(function (button) {
                switch (button) {
                case 0:
  ...
  function handleConflicts(mergeFailure, transaction, repoId, ourName, theirName, mergeBranch) {
    var myDialog = dialogService_.warn(translate_.instant('pull_conflicts'), translate_.instant('conflicts_encountered'), [
