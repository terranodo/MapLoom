(function () {
  ...
  module.directive('loomTableView', [
    ...
    'statisticsService',
    '$rootScope',
    function (tableFilter, mapService, $http, tableViewService, featureManagerService, dialogService, $translate, statisticsService, $rootScope) {
      return {
        restrict: 'C',
        ...
        link: function (scope, element) {
          ...
          scope.goToMap = function () {
            var projectedgeom = transformGeometry(scope.selectedRow.feature.geometry, tableViewService.selectedLayer.get('metadata').projection, mapService.map.getView().getProjection());
            mapService.zoomToExtent(projectedgeom.getExtent());
            var item = {
                layer: tableViewService.selectedLayer,
                features: [scope.selectedRow.feature]
              };
          ...
          scope.showStatistics = function () {
            ...
            var meta = tableViewService.selectedLayer.get('metadata');
            statisticsService.summarizeAttribute(tableViewService.selectedLayer, meta.filters, scope.selectedAttribute.name).then(function (statistics) {
          ...
          function getWfsData() {
            ...
            if (featuresModified === 1) {
              commitMessage = $translate.instant('modified_1_feature', { 'layer': tableViewService.selectedLayer.get('metadata').nativeName });
            } else {
          ...
          function postAllFeatures() {
            ...
            $http.post(url, xmlData, { headers: { 'Content-Type': 'text/xml;charset=utf-8' } }).success(function () {
              ...
              scope.isSaving = false;
              dialogService.error($translate.instant('save_attributes'), $translate.instant('failed_to_save_features'), [$translate.instant('btn_ok')], false);
