(function () {
  ...
  module.provider('tableViewService', function () {
    ...
    var getSpatialFilterXML = function (geometryGML, geometryColumn) {
      return '<ogc:Intersects>' + '<ogc:PropertyName>' + geometryColumn + '</ogc:PropertyName>' + geometryGML + '</ogc:Intersects>';
    };
    var getGeometryColumn = function (schema) {
      ...
      for (var key in schema) {
        ...
        if (contains(property._type, 'point') || contains(property._type, 'line') || contains(property._type, 'polygon') || contains(property._type, 'curve') || contains(property._type, 'surface')) {
    ...
    this.getFeaturesPostPayloadXML = function (layer, filters, bbox, resultsPerPage, currentPage, exclude_header) {
      ...
      if (goog.isDefAndNotNull(bbox)) {
        bboxStr = '<ogc:BBOX>' + '<ogc:Envelope srsName="EPSG:900913">' + '<ogc:PropertyName>BoundingBox</ogc:PropertyName>' + '<ogc:lowerCorner>' + bbox[0] + ' ' + bbox[1] + '</ogc:lowerCorner>' + '<ogc:upperCorner>' + bbox[2] + ' ' + bbox[3] + '</ogc:upperCorner>' + '</ogc:Envelope>' + '</ogc:BBOX>';
      }
      ...
      xml += '<wfs:GetFeature service="WFS" version="' + settings.WFSVersion + '"' + ' outputFormat="JSON"' + paginationParamsStr + ' xmlns:wfs="http://www.opengis.net/wfs"' + ' xmlns:ogc="http://www.opengis.net/ogc"' + ' xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"' + ' xsi:schemaLocation="http://www.opengis.net/wfs' + ' http://schemas.opengis.net/wfs/1.1.0/wfs.xsd">' + '<wfs:Query typeName="' + metadata.name + '"' + ' srsName="' + metadata.projection + '"' + '>';
      var spatialFilter = '';
    ...
    this.loadData = function () {
      ...
      var xmlData;
      xmlData = service_.getFeaturesPostPayloadXML(service_.selectedLayer, metadata.filters, null, service_.resultsPerPage, service_.currentPage);
      ...
      http_.post(postURL, xmlData, { headers: { 'Content-Type': 'text/xml;charset=utf-8' } }).success(function (data, status, headers, config) {
        ...
        var getRestrictions = function () {
          ...
          for (var attrIndex in service_.attributeNameList) {
            ...
            } else if (schemaType === 'xsd:double' || schemaType === 'xsd:decimal' || schemaType === 'xsd:long' || schemaType === 'xsd:short') {
        ...
        for (var propName in metadata.schema) {
          if (metadata.schema[propName]._type.search('gml:') === -1) {
            ...
            service_.attributeNameList.push({
              name: propName,
              filter: metadata.filters[propName]
            });
          }
        }
