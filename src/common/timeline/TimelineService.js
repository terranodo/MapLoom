(function () {
  ...
  module.provider('timelineService', function () {
    ...
    this.$get = [
      ...
      function (mapService, $interval, $rootScope, featureManagerService) {
        service_ = this;
            ...
            if (goog.isDefAndNotNull(source.updateParams)) {
              if (filterByTime_) {
                if (goog.isDefAndNotNull(timelineTicks_) && tickIndex < timelineTicks_.length && tickIndex >= 0) {
                  source.updateParams({ TIME: timelineTicks_[tickIndex] });
                }
              } else {
                source.updateParams({ TIME: '-99999999999-01-01T00:00:00.0Z/99999999999-01-01T00:00:00.0Z' });
              }
            }
          }
        }
